...
- (void)setHookDelegate:(id<HWWKWebViewHookDelegate>)hookDelegate {
    ...
    [self removeObserveValueForBlock:nil keyPath:@"title" context:@"hookDelegate"];
    
    if (hookDelegate) {
        __weak typeof(self) ws = self;
        [self observeValueForKeyPath:@"title" options:NSKeyValueObservingOptionNew context:@"hookDelegate" changeBlock:^(NSString *keyPath, id object, NSDictionary<NSKeyValueChangeKey,id> *change, void * context) {
            if (ws.hookDelegate && [(NSObject *)ws.hookDelegate respondsToSelector:@selector(webView:didReceiveNewTitle:)]) {
